...
@section('content')
<form method="POST" id="orders" action="{{ route('orders.update',$order->id) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="card-body">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    ...
                    @if(isset($order))
                        <div><img width="160px" onerror="if (this.src != '/images/invest1.png') this.src = '/images/invest1.png';" src="{{ $order->getFirstMediaUrl("image")}}"/> </div>
                    @endif
                </div>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    ...
                    @if(isset($order))
                        <div><img width="160px" onerror="if (this.src != '/images/details.jpg') this.src = '/images/details.jpg';" src="{{ $order->getFirstMediaUrl("banner")}}"/> </div>
                    @endif
                </div>
...
@section('js')
    ...
    <script>
        ...
        new Vue({
            ...
            data:{
                editorOption: {},
                title:"{{ isset($order) ? $order->title : old('title') }}",
                body:`{!! isset($order) ? $order->body : old('body') !!}`,
                excerpt:"{{ isset($order) ? $order->excerpt : old('excerpt') }}",
                category_id:"{{ isset($order) ? $order->category_id : old('category_id') }}",
                author_id:"{{ isset($order) ? $order->author_id : old('author_id') }}",
            },
            methods:{
                save(){
                    $("#body").val(this.body);
